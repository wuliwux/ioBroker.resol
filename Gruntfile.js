...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'wuliwux';
    var email     = grunt.option('email')  || 'wuliwux1@aon.at';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new resol name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/resol.png')) {
            fs.renameSync(__dirname + '/admin/resol.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/resol.html')) {
            fs.renameSync(__dirname + '/widgets/resol.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/resol/js/resol.js')) {
            fs.renameSync(__dirname + '/widgets/resol/js/resol.js', __dirname + '/widgets/resol/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/resol')) {
            fs.renameSync(__dirname + '/widgets/resol',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Resol/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Resol'
                        },
                        {
                            match: /wuliwux/g,
                            replacement: author
